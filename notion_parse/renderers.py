...
class BaseRenderer(object):
    ...
    def render_block(self, block, level=0, preblock=None, postblock=None):
        ...
        pretext = type_renderer(
            block, level=level, preblock=preblock, postblock=postblock
        )
        ...
        return (
            ...
            + self.render_children(
                block, level=level + self.calculate_child_indent(block)
            )
            + posttext
        )
    ...
    def render_children(self, block, level):
        ...
        for i in range(len(kids)):
            ...
            text += self.render_block(
                kids[i], level=level, preblock=preblock, postblock=postblock
            )
        return text
...
class BaseHTMLRenderer(BaseRenderer):
    ...
    def wrap_in_tag(self, block, tagname, fieldname="title", attributes={}):
        ...
        return "{opentag}{innerhtml}</{tagname}>".format(
            opentag=opentag, tagname=tagname, innerhtml=innerhtml
        )
    ...
    def handle_default(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block, "p", attributes=self.left_margin_for_level(level)
        )
    ...
    def handle_column(self, block, level=0, preblock=None, postblock=None):
        ...
        return (
            '<div style="flex-grow: 0; flex-shrink: 0; width: calc((100% - {}px) * {});">'.format(
                buffer, width
            ),
            "</div>",
        )
    ...
    def handle_code(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block, "code", attributes=self.left_margin_for_level(level)
        )
    ...
    def handle_header(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block, "h2", attributes=self.left_margin_for_level(level)
        )

    def handle_sub_header(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block, "h3", attributes=self.left_margin_for_level(level)
        )

    def handle_sub_sub_header(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block, "h4", attributes=self.left_margin_for_level(level)
        )

    def handle_page(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block, "h1", attributes=self.left_margin_for_level(level)
        )
    ...
    def handle_bulleted_list(self, block, level=0, preblock=None, postblock=None):
        ...
        if preblock is None or preblock.type != "bulleted_list":
            text = self.create_opening_tag(
                "ul", attributes=self.left_margin_for_level(level)
            )
    ...
    def handle_numbered_list(self, block, level=0, preblock=None, postblock=None):
        ...
        if preblock is None or preblock.type != "numbered_list":
            text = self.create_opening_tag(
                "ol", attributes=self.left_margin_for_level(level)
            )
        text += self.wrap_in_tag(block, "li")
    ...
    def handle_toggle(self, block, level=0, preblock=None, postblock=None):
        ...
        opentag = self.create_opening_tag(
            "details", attributes=self.left_margin_for_level(level)
        )
        return (
            "{opentag}<summary>{innerhtml}</summary>".format(
                opentag=opentag, innerhtml=innerhtml
            ),
            "</details>",
        )
    ...
    def handle_quote(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block, "blockquote", attributes=self.left_margin_for_level(level)
        )

    def handle_text(self, block, level=0, preblock=None, postblock=None):
        return self.handle_default(
            block=block, level=level, preblock=preblock, postblock=postblock
        )

    def handle_equation(self, block, level=0, preblock=None, postblock=None):
        text = self.create_opening_tag(
            "p", attributes=self.left_margin_for_level(level)
        )
        return (
            text
            + '<img src="https://chart.googleapis.com/chart?cht=tx&chl={}"/></p>'.format(
                block.latex
            )
        )

    def handle_embed(self, block, level=0, preblock=None, postblock=None):
        iframetag = self.create_opening_tag(
            "iframe",
            attributes={
                ...
                "allowfullscreen": "",
                "style": "width: {width}px; height: {height}px; border-radius: 1px;".format(
    ...
    def handle_bookmark(self, block, level=0, preblock=None, postblock=None):
        return bookmark_template.format(
            link=block.link,
            title=block.title,
            description=block.description,
            icon=block.bookmark_icon,
    ...
    def handle_link_to_collection(self, block, level=0, preblock=None, postblock=None):
        return self.wrap_in_tag(
            block,
            "p",
            attributes={"href": "https://www.notion.so/" + block.id.replace("-", "")},
        )
    ...
    def handle_collection_view_page(
        self, block, level=0, preblock=None, postblock=None
    ):
        return self.wrap_in_tag(
            block,
            "p",
    ...
    def handle_callout(self, block, level=0, preblock=None, postblock=None):
        return callout_template.format(
            icon=block.icon, title=markdown2.markdown(block.title)
        )
